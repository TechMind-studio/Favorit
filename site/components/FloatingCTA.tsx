...
export default function FloatingCTA() {
  ...
  return (
    <>
      <div
        ref={wrapRef}
        className={`fixed bottom-6 right-6 md:bottom-24 md:right-24 z-40 flex flex-col items-center gap-3 transition-all duration-300 ${
      ...
      >
        {actions.map((action, i) => (
          <a
            ...
            className={`${action.bg} text-white w-12 h-12 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ${
              open
                ? 'opacity-100 translate-y-0 scale-100'
                : 'opacity-0 translate-y-4 scale-75 pointer-events-none'
            }`}
            style={{ transitionDelay: open ? `${i * 50}ms` : `${(actions.length - i) * 30}ms` }}
          >
            {action.icon}
          </a>
        ))}
        ...
        <button
          ...
          className="w-14 h-14 md:w-20 md:h-20 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg shadow-green-500/40 flex items-center justify-center transition-all duration-300 active:scale-95"
        >
          <svg
            className={`w-6 h-6 md:w-9 md:h-9 transition-transform duration-300 ${open ? 'rotate-45' : 'rotate-0'}`}
